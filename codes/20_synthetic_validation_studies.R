# ============================================================
# 20_formalism_validation_real.R
# ============================================================
# Description:
...
generate_real_validation <- function(df_real,
  ...
  for (i in seq_len(nrow(df_real))) {
    ...
    results[[i]] <- tibble(
      ...
      psi_form_median = quantile(out_form, 0.5),
      psi_form_lower  = quantile(out_form, 0.25),
      psi_form_upper  = quantile(out_form, 0.75),
      psi_form_delta  = (study$psi_c - quantile(out_form, 0.5)) * 100 / study$psi_c,
      psi_form_width  = diff(quantile(out_form, c(0.25, 0.75)))
    )
  }
